...
MessageLoop& MessageLoop::GetCurrent() {
  ...
  DEBUGROUTER_BASE_CHECK(loop != nullptr);
...
MessageLoop::MessageLoop(void* platform_loop)
  ...
  // TODO(zhengsenyao): Replace DEBUGROUTER_BASE_CHECK with CHECK when CHECK available.
  DEBUGROUTER_BASE_CHECK(loop_);
  DEBUGROUTER_BASE_CHECK(task_runner_);
...
void MessageLoop::UnBind(const TaskQueueId& queue_id) {
  ...
  DEBUGROUTER_BASE_CHECK(queue_id != task_runner_->GetTaskQueueId());
