...
#include "debug_router/native/harmony/base/fml/make_copyable.h"
#include "debug_router/native/harmony/base/fml/message_loop_task_queues.h"
...
bool MessageLoopTaskQueues::Merge(TaskQueueId owner, TaskQueueId subsumed) {
  ...
  if (owner_entry->subsumed_by != _kUnmerged) {
    // TODO: Uncomment LOGW code when LOGW available.
    // LOGW(
...
TaskSource::TopTask MessageLoopTaskQueues::PeekNextTaskUnlocked(
  ...
  if (entry->owner_of.empty()) {
    // TODO: Replace DEBUGROUTER_BASE_CHECK with CHECK when CHECK
    // available.
    DEBUGROUTER_BASE_CHECK(!entry->task_source->IsEmpty());
  ...
  // TODO: Replace DEBUGROUTER_BASE_CHECK with CHECK when CHECK available.
  DEBUGROUTER_BASE_CHECK(top_task.has_value());
  return *top_task;
}
