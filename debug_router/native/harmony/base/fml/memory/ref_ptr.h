...
#ifndef DEBUGROUTER_HARMONY_DEBUGROUTER_SRC_MAIN_BASE_FML_MEMORY_REF_PTR_H_
#define DEBUGROUTER_HARMONY_DEBUGROUTER_SRC_MAIN_BASE_FML_MEMORY_REF_PTR_H_

#include <cstddef>
#include <functional>
#include <utility>

#include "debug_router/native/harmony/base/fml/memory/ref_ptr_internal.h"
...
class RefPtr final {
 public:
  ...
  T& operator*() const {
    // TODO: Uncomment DCHECK code when DCHECK available.
    // DCHECK(ptr_);
    return *ptr_;
  }
...
namespace std {
template <typename T>
struct hash<debugrouter::fml::RefPtr<T>> {
  using argument_type = debugrouter::fml::RefPtr<T>;
...
namespace fml {
using debugrouter::fml::AdoptRef;
using debugrouter::fml::MakeRefCounted;
using debugrouter::fml::Ref;
using debugrouter::fml::RefPtr;
using debugrouter::fml::WeakRefPtr;
}  // namespace fml

#endif  // DEBUGROUTER_HARMONY_DEBUGROUTER_SRC_MAIN_BASE_FML_MEMORY_REF_PTR_H_
