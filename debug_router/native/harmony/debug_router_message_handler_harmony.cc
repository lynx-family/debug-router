...
std::string DebugRouterMessageHandlerHarmony::Handle(std::string params) {
  ...
  ui_task_runner->PostTask([weak_ptr = weak_from_this(), &ret, params]() {
    ...
    napi_value handle;
    auto status = napi_get_named_property(handler->env_, js_this, "handle", &handle);

    napi_value args[1];
    napi_create_string_utf8(handler->env_, params.c_str(), NAPI_AUTO_LENGTH, &args[0]);

    napi_value result;
    status = napi_call_function(handler->env_, js_this, handle, 1, args, &result);

    size_t strSize;
    status = napi_get_value_string_utf8(handler->env_, result, nullptr, 0, &strSize);
    char* buffer = new char[strSize + 1];
    status = napi_get_value_string_utf8(handler->env_, result, buffer, strSize + 1, &strSize);
...
std::string DebugRouterMessageHandlerHarmony::GetName() const {
  ...
  ui_task_runner->PostTask([weak_ptr = weak_from_this(), &ret]() {
    ...
    napi_get_reference_value(handler->env_, handler->js_this_ref_, &js_this);
    napi_value getName;
    auto status = napi_get_named_property(handler->env_, js_this, "getName", &getName);

    napi_value result;
    status = napi_call_function(handler->env_, js_this, getName, 0, nullptr, &result);
