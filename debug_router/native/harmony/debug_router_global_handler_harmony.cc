...
void DebugRouterGlobalHandlerHarmony::OpenCard(const std::string &url) {
  ...
  ui_task_runner->PostTask([weak_ptr = weak_from_this(), url]() {
    ...
    napi_value openCard;
    auto status = napi_get_named_property(handler->env_, js_this, "openCard", &openCard);

    napi_value args[1];
    napi_create_string_utf8(handler->env_, url.c_str(), NAPI_AUTO_LENGTH, &args[0]);

    napi_value result;
    status = napi_call_function(handler->env_, js_this, openCard, 1, args, &result);
  });
}
