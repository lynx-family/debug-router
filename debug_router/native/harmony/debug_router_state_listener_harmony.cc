...
void DebugRouterStateListenerHarmony::OnOpen(debugrouter::core::ConnectionType type) {
  ...
  ui_task_runner->PostTask([weak_ptr = weak_from_this(), type]() {
    ...
    napi_value onOpen;
    auto status = napi_get_named_property(handler->env_, js_this, "onOpen", &onOpen);

    napi_value args[1];
    napi_create_string_utf8(handler->env_, debugrouter::core::ConnectionTypes[type].c_str(), NAPI_AUTO_LENGTH, &args[0]);

    napi_value result;
    status = napi_call_function(handler->env_, js_this, onOpen, 1, args, &result);
  });
}
...
void DebugRouterStateListenerHarmony::OnClose(int32_t code, const std::string &reason) {
  ...
  ui_task_runner->PostTask([weak_ptr = weak_from_this(), code, reason]() {
    ...
    napi_get_reference_value(handler->env_, handler->js_this_ref_, &js_this);
    napi_value onClose;
    auto status = napi_get_named_property(handler->env_, js_this, "onClose", &onClose);

    napi_value args[2];
    napi_create_int32(handler->env_, code, &args[0]);
    napi_create_string_utf8(handler->env_, reason.c_str(), NAPI_AUTO_LENGTH, &args[1]);

    napi_value result;
    status = napi_call_function(handler->env_, js_this, onClose, 2, args, &result);
  });
}
...
void DebugRouterStateListenerHarmony::OnMessage(const std::string &message) {
  ...
  ui_task_runner->PostTask([weak_ptr = weak_from_this(), message]() {
    ...
    napi_value result;
    status = napi_call_function(handler->env_, js_this, onMessage, 1, args, &result);
  });
}
...
void DebugRouterStateListenerHarmony::OnError(const std::string &error) {
  ...
  ui_task_runner->PostTask([weak_ptr = weak_from_this(), error]() {
    ...
    napi_get_reference_value(handler->env_, handler->js_this_ref_, &js_this);
    napi_value onError;
    auto status = napi_get_named_property(handler->env_, js_this, "onError", &onError);

    napi_value args[1];
    napi_create_string_utf8(handler->env_, error.c_str(), NAPI_AUTO_LENGTH, &args[0]);

    napi_value result;
    status = napi_call_function(handler->env_, js_this, onError, 1, args, &result);
  });
}
