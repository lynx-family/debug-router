...
std::map<napi_value, std::shared_ptr<DebugRouterGlobalHandlerHarmony>, NapiValueCompare> DebugRouterHarmony::global_handlers;
std::map<napi_value, std::shared_ptr<DebugRouterSessionHandlerHarmony>, NapiValueCompare> DebugRouterHarmony::session_handlers;
std::map<napi_value, std::shared_ptr<DebugRouterMessageHandlerHarmony>, NapiValueCompare> DebugRouterHarmony::message_handlers;
...
napi_value DebugRouterHarmony::Init(napi_env env, napi_value exports) {
  napi_property_descriptor properties[] = {
      ...
      {"setAppInfo", 0, DebugRouterHarmony::SetAppInfo, 0, 0, 0, napi_static, 0},
      {"getAppInfoByKey", 0, DebugRouterHarmony::GetAppInfoByKey, 0, 0, 0, napi_static, 0}
    };
...
napi_value DebugRouterHarmony::SetAppInfo(napi_env env, napi_callback_info info) {
  napi_value js_this;
  size_t argc = 2;
  napi_value argv[2];
  napi_get_cb_info(env, info, &argc, argv, &js_this, nullptr);
  ...
  if (argc == 1) {
    ...
    if (value_type != napi_object) {
      napi_throw_error(env, nullptr, "First argument must be an object when single parameter is provided");
      return nullptr;
    }
...
napi_value DebugRouterHarmony::GetAppInfoByKey(napi_env env, napi_callback_info info) {
  napi_value js_this;
  size_t argc = 1;
  napi_value argv[1];
  napi_get_cb_info(env, info, &argc, argv, &js_this, nullptr);
