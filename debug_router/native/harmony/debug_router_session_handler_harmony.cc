...
void DebugRouterSessionHandlerHarmony::OnSessionCreate(int session_id, const std::string &url) {
  ...
  ui_task_runner->PostTask([weak_ptr = weak_from_this(), session_id, url]() {
    ...
    auto status = napi_get_named_property(handler->env_, js_this, "onSessionCreate", &onSessionCreate);

    napi_value args[2];
    napi_create_int32(handler->env_, session_id, &args[0]);
    napi_create_string_utf8(handler->env_, url.c_str(), NAPI_AUTO_LENGTH, &args[1]);

    napi_value result;
    status = napi_call_function(handler->env_, js_this, onSessionCreate, 2, args, &result);
  });
}
...
void DebugRouterSessionHandlerHarmony::OnSessionDestroy(int session_id) {
  ...
  ui_task_runner->PostTask([weak_ptr = weak_from_this(), session_id]() {
    ...
    auto status = napi_get_named_property(handler->env_, js_this, "onSessionDestroy", &onSessionDestroy);

    napi_value args[1];
    napi_create_int32(handler->env_, session_id, &args[0]);

    napi_value result;
    status = napi_call_function(handler->env_, js_this, onSessionDestroy, 1, args, &result);
  });
 }
...
void DebugRouterSessionHandlerHarmony::OnMessage(const std::string &message, const std::string &type, int session_id) {
  auto ui_task_runner =
      fml::MessageLoop::EnsureInitializedForCurrentThread(loop_)
          .GetTaskRunner();
  ui_task_runner->PostTask([weak_ptr = weak_from_this(), message, type, session_id]() {
    napi_value js_this;
    auto handler = weak_ptr.lock();
    if (!handler) {
      return;
    ...
    napi_value result;
    status = napi_call_function(handler->env_, js_this, onMessage, 3, args, &result);
  });
}
