...
""")
    ...
    if self.options.native_exports_optional:
      early_exit = """\
  if (debugrouter::common::android::IsManualJniRegistrationDisabled()) return true;
"""
...
"""
  ...
  def GetCalledByNativeValues(self, called_by_native):
    ...
    if not called_by_native.unchecked:
      check_exception = 'debugrouter::common::android::CheckException(env);'
    ...
    if return_type != 'void':
      ...
      if re.match(RE_SCOPED_JNI_RETURN_TYPES, return_type):
        return_type = 'debugrouter::common::android::ScopedLocalJavaRef<' + return_type + '>'
...
debugrouter::common::subtle::AtomicWord g_${JAVA_CLASS}_clazz __attribute__((unused)) = 0;
#define ${JAVA_CLASS}_clazz(env) \
debugrouter::common::android::LazyGetClass(env, k${JAVA_CLASS}ClassPath, \
...
#define ${JAVA_CLASS}_clazz(env) g_${JAVA_CLASS}_clazz""")
  ...
  def GetFindClasses(self):
    ...
    if self.init_native:
      if self.options.native_exports:
        template = Template("""\
  debugrouter::common::subtle::Release_Store(&g_${JAVA_CLASS}_clazz,
      static_cast<debugrouter::common::subtle::AtomicWord>(env->NewWeakGlobalRef(clazz));""")
      else:
        template = Template("""\
  ...
  g_${JAVA_CLASS}_clazz = reinterpret_cast<jclass>(env->NewGlobalRef(
      debugrouter::common::android::GetClass(env, k${JAVA_CLASS}ClassPath).Get()));""")
    ret = []
...
env->Get${STATIC_METHOD_PART}MethodID(
      ...
      template = Template("""\
  debugrouter::common::android::GetMethod(
      env, ${JAVA_CLASS}_clazz(env),
      debugrouter::common::android::${STATIC}_METHOD,
      "${JNI_NAME}",
      ${JNI_SIGNATURE},
