...
# Regex to match a string like "@ReactMethod public void foo(int bar)".
RE_CALLED_BY_NATIVE = re.compile(
    '@ReactMethod(?P<Unchecked>(Unchecked)*?)(?:\("(?P<annotation>.*)"\))?'
    '\s+(?P<prefix>[\w ]*?)'
    '\s*(?P<return_type>\S+?)'
    '\s+(?P<name>\w+)'
    '\s*\((?P<params>[^\)]*)\)')
...
def ExtractCalledByNatives(contents):
  """Parses all methods annotated with @ReactMethod.
  ...
  for match in re.finditer(RE_CALLED_BY_NATIVE, contents):
    called_by_natives += [CalledByNative(
        ...
        params=JniParams.Parse(match.group('params')))]
  # Check for any @ReactMethod occurrences that weren't matched.
  unmatched_lines = re.sub(RE_CALLED_BY_NATIVE, '', contents).split('\n')
  for line1, line2 in zip(unmatched_lines, unmatched_lines[1:]):
    if '@ReactMethod' in line1:
      raise ParseError('could not parse @ReactMethod method signature',
                       line1, line2)
