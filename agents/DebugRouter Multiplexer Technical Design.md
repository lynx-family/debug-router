...
| Multiplexer daemon          | A local detached shared process composed of the process entry and `MultiplexerHost`. It is a deployment role rather than a standalone lifecycle class.                                                                                         |
...
The Multiplexer daemon is a process boundary, not an additional object layer. `entry.ts` adapts process startup and termination to one `MultiplexerHost`; Host composes and owns the control, physical, WebSocket, routing, and compatibility resources used by that process.
...
`entry.ts` is the adapter between process lifecycle and the daemon runtime. It parses Manager-provided configuration, installs daemon-local dependencies, constructs one `MultiplexerHost`, starts it, and coordinates process termination when Host or the operating environment requests shutdown.
...
3. `MultiplexerHost` owns startup, shutdown, and all runtime resources inside the daemon process.
